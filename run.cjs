...
const init = async (prt) => {
    ...
    MARKET = require(path.resolve('./dist/888.market/hunt'));
    MARKET_ACTION = require(path.resolve('./dist/888.market/00.market.unit/market.action'));
    ...
    if ( pvt == false){
      ...
      await MARKET.hunt( MARKET_ACTION.INIT_MARKET , { val: 1, dat: MQTT, src:  [localBit]  });
